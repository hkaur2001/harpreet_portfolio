...
The native application uses OpenAI Responses API function calling and strict structured output. Tool choice and analysis are agentic; allowlists, argument validation, budgets, inspected-source citations, numeric/unit checks, and write authority are enforced in code. The research desk reads six synthetic financial sources; the deployment planner reads four synthetic industry scenarios, not customer systems. Neither has external-write tools or persists briefs. Provider failures or unsupported outputs fail visibly instead of substituting a scripted brief or plan.
