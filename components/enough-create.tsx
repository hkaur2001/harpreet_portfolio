  ...
  return (
    <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <section className="rounded-[2rem] border border-[#2a2723]/10 bg-[#1e1b18] p-6 text-[#fffaf3] shadow-[0_24px_80px_rgba(38,31,24,0.14)] md:p-8">
        ...
        <div className="mt-6"><div className="flex items-center justify-between"><p className="text-sm font-semibold">Possible times</p><button type="button" disabled={times.length >= 4} onClick={() => setTimes((current) => [...current, { value: current.at(-1)?.value || "" }])} className="text-xs font-bold text-[#cfc1ff] disabled:opacity-35">+ add time</button></div><div className="mt-2 grid gap-2 sm:grid-cols-2">{times.map((item, index) => <div key={index} className="flex items-end gap-2"><label className="min-w-0 flex-1 text-xs font-semibold text-white/75">Time option {index + 1}<input required type="datetime-local" value={item.value} onChange={(e) => updateTime(index, e.target.value)} className="mt-1 w-full rounded-2xl border border-white/15 bg-white/8 px-3 py-3 text-sm text-white outline-none [color-scheme:dark] focus:border-[#cfc1ff]" /></label>{times.length > 1 && <button type="button" aria-label={`Remove time ${index + 1}`} onClick={() => setTimes((current) => current.filter((_, i) => i !== index))} className="min-h-11 rounded-xl border border-white/10 px-3 text-white/70">×</button>}</div>)}</div></div>

        <div className="mt-5"><div className="flex items-center justify-between"><p className="text-sm font-semibold">Possible places <span className="font-normal text-white/45">Optional</span></p><button type="button" disabled={places.length >= 4} onClick={() => setPlaces((current) => [...current, { value: "" }])} className="text-xs font-bold text-[#cfc1ff] disabled:opacity-35">+ add place</button></div><div className="mt-2 grid gap-2 sm:grid-cols-2">{places.map((item, index) => <div key={index} className="flex items-end gap-2"><label className="min-w-0 flex-1 text-xs font-semibold text-white/75">Place option {index + 1}<input maxLength={160} value={item.value} onChange={(e) => updatePlace(index, e.target.value)} className="mt-1 w-full rounded-2xl border border-white/15 bg-white/8 px-3 py-3 text-sm text-white outline-none placeholder:text-white/25 focus:border-[#cfc1ff]" placeholder={index === 0 ? "East Village" : "Williamsburg"} /></label>{places.length > 1 && <button type="button" aria-label={`Remove place ${index + 1}`} onClick={() => setPlaces((current) => current.filter((_, i) => i !== index))} className="min-h-11 rounded-xl border border-white/10 px-3 text-white/70">×</button>}</div>)}</div></div>
