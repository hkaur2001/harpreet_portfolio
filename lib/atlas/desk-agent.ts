...
export async function runDeskAgent(input: DeskInput): Promise<DeskResult> {
  ...
  async function call(extra: Record<string, unknown>) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        const result = await fetchJsonWithRetry<ModelResponse>(openAiUrl("responses"), { method: "POST", headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({ model, instructions, input: messages, reasoning: { effort: "low" }, max_output_tokens: 2200, store: false, ...extra }) }, { attempts: 1, timeoutMs: Math.min(15000, remaining) });
