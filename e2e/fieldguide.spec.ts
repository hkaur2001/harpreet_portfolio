...
test.describe("FieldGuide deployment workbench", () => {
  test("portfolio navigation and homepage expose FieldGuide", async ({ page }) => {
    ...
    const fieldGuide = page.getByRole("article").filter({ has: page.getByRole("heading", { name: "FieldGuide", exact: true }) });
    await expect(fieldGuide.getByRole("link", { name: /Open project/i })).toHaveAttribute("href", "/projects/fieldguide");
  });
