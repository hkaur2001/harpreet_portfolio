...
export async function POST(request: Request) {
  ...
  try {
    ...
    const billing = error instanceof UpstreamRequestError && error.status === 402;
    ...
    const message = billing
      ? "Live model billing or quota is unavailable. The investigation cannot run until provider access is restored; no scripted brief was substituted."
      : busy
        ? `All live model routes are at capacity. Please wait ${retrySeconds} seconds and retry; no scripted brief was substituted.`
        : "The agent could not produce a complete, source-backed brief within its limits. Retry with a simpler question; no unsupported brief was returned.";
    return NextResponse.json({ error: message, code: error instanceof UpstreamRequestError ? `PROVIDER_${error.status}` : error instanceof Error && ["TIME_BUDGET", "TOOL_BUDGET", "EVIDENCE_INCOMPLETE", "BRIEF_INVALID"].includes(error.message) ? error.message : "AGENT_FAILED" }, { status: 502, headers: { "Cache-Control": "no-store", ...(busy ? { "Retry-After": String(retrySeconds) } : {}) } });
  }
}
